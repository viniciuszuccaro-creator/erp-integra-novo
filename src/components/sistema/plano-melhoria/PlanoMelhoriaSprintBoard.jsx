...
import { ciclo17Items, ciclo18Items } from './melhoriaPlanData';
...
export default function PlanoMelhoriaSprintBoard() {
  const [cicloView, setCicloView] = useState('c18');
  const activeItems = cicloView === 'c18' ? ciclo18Items : ciclo17Items;
  const cicloLabel = cicloView === 'c18' ? 'Ciclo 18 — Dezembro 2026' : 'Ciclo 17 — Novembro 2026';
    ...
    <div className="w-full space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="ml-auto flex gap-2">
          <button
            onClick={() => setCicloView('c17')}
            className={`text-xs px-3 py-1 rounded-full border transition-colors ${cicloView === 'c17' ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-300 text-slate-600 hover:bg-slate-50'}`}
          >
            Ciclo 17
          </button>
          <button
            onClick={() => setCicloView('c18')}
            className={`text-xs px-3 py-1 rounded-full border transition-colors ${cicloView === 'c18' ? 'bg-purple-600 text-white border-purple-600' : 'border-slate-300 text-slate-600 hover:bg-slate-50'}`}
          >
            Ciclo 18 ▶
          </button>
        </div>
