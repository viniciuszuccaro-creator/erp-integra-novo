/**
 * Fase5StatusCard — Checklist visual da Fase 5: Marketplace & Integrações
 */
import React, { useState, useEffect, useCallback } from "react";
import {
  CheckCircle2, XCircle, Loader2, RefreshCw, Globe,
...
const ITEM_META = {
  gestor_centralizado_marketplaces: { label: "Gestor centralizado de marketplaces",               group: "Marketplace"  },
  sync_realtime_marketplace:        { label: "Sync realtime — estoque/preços/pedidos",            group: "Sync"         },
  webhook_handler_retry:            { label: "Webhook handler robusto com retry",                 group: "Webhooks"     },
  rate_limiting:                    { label: "Rate limiting — proteção anti-burst",               group: "Segurança"    },
  circuit_breaker:                  { label: "Circuit breaker — resiliência de integrações",      group: "Segurança"    },
  dashboard_integracoes:            { label: "Dashboard de integrações — CentralIntegracoes",     group: "Dashboard"    },
  pedido_externo_sync:              { label: "PedidoExterno — sync de pedidos externos",          group: "Sync"         },
  catalogo_ecommerce:               { label: "Catálogo web + e-commerce integrado",               group: "Marketplace"  },
  seguranca_integracoes:            { label: "Segurança — API keys + RBAC + sanitização",         group: "Segurança"    },
  auditoria_integracoes:            { label: "Auditoria completa de integrações multiempresa",    group: "Auditoria"    },
};

const GROUP_ORDER = ["Marketplace", "Sync", "Webhooks", "Segurança", "Dashboard", "Auditoria"];
const GROUP_COLORS = {
...
function StatusRow({ item }) {
  ...
  const meta = ITEM_META[item.id] || { label: item.id, group: "Marketplace" };
  return (
...
export default function Fase5StatusCard() {
  ...
  const runChecks = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await base44.functions.invoke("fase5Check", {});
      const data = res?.data;
  ...
  const grouped = GROUP_ORDER.map(g => ({
    group: g,
    items: items.filter(i => (ITEM_META[i.id]?.group || "Marketplace") === g),
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <CardTitle className="text-sm font-semibold text-slate-800">
                Fase 5 — Marketplace & Integrações
              </CardTitle>
      ...
      <CardContent className="flex-1 overflow-auto px-4 pb-4">
        {loading && items.length === 0 && (
          <div className="flex items-center justify-center py-8 gap-2 text-slate-400">
            ...
            <span className="text-sm">Verificando marketplace & integrações…</span>
          </div>
        )}
        {error && (
        ...
        {score === 100 && !loading && (
          <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg mb-3">
            ...
            <div>
              <p className="text-sm font-semibold text-green-700">Fase 5 concluída com 100%!</p>
              <p className="text-xs text-green-600">Marketplace, sync realtime, webhooks, circuit breaker e dashboard de integrações ativos.</p>
            </div>
          </div>
        )}
