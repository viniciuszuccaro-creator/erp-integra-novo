 ...
 */
export default function CriarEtapaEntregaForm({ pedidoData, onCriarEtapa, windowMode = false }) {
            ...
            {itensDisponiveis.map((item) => (
              <div
                ...
                className={`p-3 rounded border cursor-pointer transition-all ${
                  etapaForm.itens_selecionados.includes(item.id) ? 'bg-blue-50 border-blue-300' : 'bg-white hover:bg-slate-50'
                }`}
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-600 mt-1">
                        ...
                        <Badge variant="outline" className="text-xs">
                          {item.origem === 'revenda' ? '🛒 Revenda' :
                           item.origem === 'armado_padrao' ? '🏗️ Armado' : '✂️ Corte/Dobra'}
                        </Badge>
                      </div>
                    </div>
                  </div>
