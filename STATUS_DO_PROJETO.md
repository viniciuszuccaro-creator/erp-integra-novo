...
### Testes de sanidade pós-refatoração (2026-09-06)
- Suíte unitária: 209 pass · 0 fail (estrutura de entidades, RBAC, rotas, layout responsivo, exclusão lógica).
- Suíte E2E estática: 37 pass · 0 fail (8 fluxos definidos).
- Testes de integração live (API real): pendentes de execução no runtime Vite — bloqueados pela indisponibilidade de créditos do workspace até 2026-09-07 (limitação de billing, não bug).
...
### Auditoria RLS completa — regra `create` obrigatória em 100% das entidades (2026-09-06)
- Varredura completa das ~155 entidades do ERP identificou entidades com políticas RLS de `read`/`update`/`delete` mas SEM a regra `create`, deixando a criação de registros sem validação de grupo multiempresa.
- **~126 entidades reescritas** com a regra `create` adicionada, espelhando exatamente as regras existentes de `read`/`update`/`delete`: operacional (Pedido, Cliente, Produto, Fornecedor, NotaFiscal, ContaPagar/ContaReceber, OrdemCompra, OrdemProducao, Entrega, Romaneio, Comissao, Inventario, ApontamentoProducao, MovimentacaoEstoque, Veiculo, Turno, Cargo, Ponto, Evento, Interacao, FormaPagamento, CentroCusto, RotaPadrao, InspecaoQualidade, ImportacaoXMLNFe, PagamentoOmnichannel, SolicitacaoCompra, SeparacaoConferencia, parametros etc.) e de suporte/sistema (configs, parâmetros, operador de caixa, conta bancária, boletos, gateways, templates WhatsApp, canais, modelos logísticos, conciliação, região de atendimento, base de conhecimento).
- **~30 entidades verificadas e já conformes** (GrupoEmpresarial, IAConfig, PerfilAcesso, PermissaoEmpresaModulo, Webhook, ApiExterna, ConfiguracaoSistema/Backup/Monitoramento/Seguranca, Motorista, MonitoramentoRH, ConciliacaoPedido, AuditLog e demais logs de auditoria, SyncMap, SyncReport, BackupAutomatico, TokenRefresh, SessaoUsuario, JobAgendado, ChatbotIntent/Intents/Canal, MonitoramentoSistema, AuditoriaIA/GPS/Acesso/Global, LogPerformance, LogsIA, AlertaPerformance, LogFiscal, LogCobranca) — sem alteração.
