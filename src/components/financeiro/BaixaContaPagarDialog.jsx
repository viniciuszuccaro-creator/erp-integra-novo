...
export default function BaixaContaPagarDialog({
  open,
  onOpenChange,
  contaAtual,
  contasSelecionadas = [],
  ...
  const valorTotal = contaAtual
    ? (contaAtual.valor || 0) + (dadosBaixa.juros || 0) + (dadosBaixa.multa || 0) - (dadosBaixa.desconto || 0)
    : 0;
      ...
      <DialogContent className="max-w-2xl">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <Label>Juros (R$)</Label>
              <Input type="number" step="0.01" value={dadosBaixa.juros}
                onChange={(e) => setDadosBaixa({ ...dadosBaixa, juros: parseFloat(e.target.value) || 0 })} />
            </div>
            <div>
              <Label>Multa (R$)</Label>
              <Input type="number" step="0.01" value={dadosBaixa.multa}
                onChange={(e) => setDadosBaixa({ ...dadosBaixa, multa: parseFloat(e.target.value) || 0 })} />
            </div>
            <div>
              <Label>Desconto (R$)</Label>
              <Input type="number" step="0.01" value={dadosBaixa.desconto}
