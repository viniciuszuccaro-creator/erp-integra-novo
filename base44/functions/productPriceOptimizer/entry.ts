...
async function assertPermission(base44, ctx, module, entity, action) { try { const res = await base44.functions.invoke('entityGuard', { module, section: entity, action }); if (res?.data && res.data.allowed === false) return Response.json({ error: 'Forbidden' }, { status: 403 }); } catch (_) { console.error('[productPriceOptimizer] catch:', _); } return null; }
async function audit(base44, user, { acao = 'Ação', modulo = 'Sistema', entidade = '-', registro_id = null, descricao = '', dados_novos = null, empresa_id = null, duracao_ms = null }) { try { await base44.asServiceRole.entities.AuditLog.create({ usuario: user?.full_name || user?.email || 'Sistema', usuario_id: user?.id, acao, modulo, entidade, registro_id, descricao, empresa_id: empresa_id || null, duracao_ms: typeof duracao_ms === 'number' ? duracao_ms : null, dados_novos: dados_novos || null, data_hora: new Date().toISOString() }); } catch (e) { console.error('[productPriceOptimizer] catch:', e); } }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (isBatch) {
      ...
      } catch (e) { console.error('[productPriceOptimizer] catch:', e); }
