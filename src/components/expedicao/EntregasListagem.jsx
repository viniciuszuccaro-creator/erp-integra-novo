              ...
              {filteredEntregas.map((entrega) => {
                ...
                return (
                  <TableRow key={entrega.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        <RBACButton variant="ghost" size="icon" module="Expedicao" action="visualizar" onClick={() => openWindow(DetalhesEntregaView, { entrega, estaNoGrupo, obterNomeEmpresa, statusColors, windowMode: true }, { title: `🚚 ${entrega.numero_pedido}`, width: 1000, height: 700 })} className="h-7 w-7">
                          <Eye className="w-3 h-3" />
                        </RBACButton>
                        {hasPermission('Expedição','Entrega','editar') && (
                          <RBACButton variant="ghost" size="icon" module="Expedicao" action="editar" onClick={() => openWindow(FormularioEntrega, { formData: entrega, windowMode: true, isEditing: true }, { title: `✏️ ${entrega.numero_pedido}`, width: 1100, height: 650 })} className="h-7 w-7">
                            <Edit className="w-3 h-3" />
                          </RBACButton>
                        )}
                      </div>
