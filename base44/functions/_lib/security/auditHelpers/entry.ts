// Helpers de auditoria e segurança — CONSOLIDADO com centralizedAuditLogger
// Fonte única de verdade: mesmas funções, mesma lógica, mesma assinatura.
// Este módulo é um mirror completo de centralizedAuditLogger para reuso em automações.

// Mapper de entidade → módulo (para RBAC/auditoria) — sincronizado com centralizedAuditLogger
...
// Safe trim para payloads grandes — recursivo e robusto (sincronizado com centralizedAuditLogger)
...
// Função utilitária centralizada para auditoria — mirror de centralizedAuditLogger
...
// Calcula risco de mudança — mirror de centralizedAuditLogger
...
// Detecta ação de negócio (Aprovação, Pagamento, etc) — mirror de centralizedAuditLogger
...
Deno.serve(async (req) => {
  return Response.json({
    ok: true, status: 'healthy', module: '_lib/security/auditHelpers',
    consolidated: true, source: 'centralizedAuditLogger'
  });
});