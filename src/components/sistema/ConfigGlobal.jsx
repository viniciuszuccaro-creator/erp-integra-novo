              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input key={`cfop-int-${eId}-${gId}`} defaultValue={getConfig('fiscal_cfop_interno')?.valor || '5102'}
                    placeholder="5102" disabled={!hasValidScope || !!savingField.fiscal_cfop_interno} onBlur={(e) => handleFieldBlur('fiscal_cfop_interno', 'Fiscal', { valor: e.target.value }, getConfig('fiscal_cfop_interno') || { valor: '5102' })} />
                </div>
                <div>
                  <Label>CFOP Padrão — Fora do Estado</Label>
                  <Input key={`cfop-ext-${eId}-${gId}`} defaultValue={getConfig('fiscal_cfop_externo')?.valor || '6102'}
                    placeholder="6102" disabled={!hasValidScope || !!savingField.fiscal_cfop_externo} onBlur={(e) => handleFieldBlur('fiscal_cfop_externo', 'Fiscal', { valor: e.target.value }, getConfig('fiscal_cfop_externo') || { valor: '6102' })} />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input type="number" key={`icms-${eId}-${gId}`} defaultValue={getConfig('fiscal_aliq_icms')?.numero || 18}
                    disabled={!hasValidScope || !!savingField.fiscal_aliq_icms} onBlur={(e) => handleFieldBlur('fiscal_aliq_icms', 'Fiscal', { numero: e.target.value === '' ? 18 : Number(e.target.value) }, getConfig('fiscal_aliq_icms') || { numero: 18 })} />
                </div>
                <div>
                  <Label>Alíquota PIS (%)</Label>
                  <Input type="number" key={`pis-${eId}-${gId}`} defaultValue={getConfig('fiscal_aliq_pis')?.numero || 1.65}
                    disabled={!hasValidScope || !!savingField.fiscal_aliq_pis} onBlur={(e) => handleFieldBlur('fiscal_aliq_pis', 'Fiscal', { numero: e.target.value === '' ? 1.65 : Number(e.target.value) }, getConfig('fiscal_aliq_pis') || { numero: 1.65 })} />
                </div>
                <div>
                  <Label>Alíquota COFINS (%)</Label>
                  <Input type="number" key={`cofins-${eId}-${gId}`} defaultValue={getConfig('fiscal_aliq_cofins')?.numero || 7.6}
                    disabled={!hasValidScope || !!savingField.fiscal_aliq_cofins} onBlur={(e) => handleFieldBlur('fiscal_aliq_cofins', 'Fiscal', { numero: e.target.value === '' ? 7.6 : Number(e.target.value) }, getConfig('fiscal_aliq_cofins') || { numero: 7.6 })} />
                </div>
              </div>
              <div>
