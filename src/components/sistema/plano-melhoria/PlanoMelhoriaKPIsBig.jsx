...
import { TrendingUp, CheckCircle2, Bot, Building2, Shield, Zap, Rocket, BarChart3 } from 'lucide-react';
...
const PILARES = [
  { key: 'multiempresa', label: 'Multiempresa', valor: 97, icon: Building2, cor: 'blue' },
  { key: 'rbac', label: 'RBAC + SoD', valor: 98, icon: Shield, cor: 'slate' },
  { key: 'ia', label: 'IA Operacional', valor: 98, icon: Bot, cor: 'purple' },
  { key: 'performance', label: 'Performance', valor: 96, icon: Zap, cor: 'amber' },
  { key: 'auditoria', label: 'Auditoria', valor: 99, icon: CheckCircle2, cor: 'emerald' },
  { key: 'modularizacao', label: 'Modularização', valor: 98, icon: BarChart3, cor: 'cyan' },
];
    ...
    <div className="w-full space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Card className="border-violet-100 bg-gradient-to-br from-violet-50 to-purple-50">
          <CardContent className="p-4 text-center">
            <CheckCircle2 className="mx-auto mb-1 h-5 w-5 text-violet-600" />
            <p className="text-4xl font-black text-violet-700">{modulosTop}</p>
            <p className="text-xs text-violet-600 mt-0.5">Módulos ≥97%</p>
          </CardContent>
        </Card>
      ...
      <Card className="w-full">
        <CardContent className="p-4">
          <p className="text-xs font-bold uppercase text-slate-500 tracking-wide mb-3">Scores por pilar</p>
              ...
              return (
                <div key={pilar.key} className="flex flex-col gap-2 rounded-xl border border-slate-100 bg-slate-50 p-3">
                  <div className="flex items-center gap-2">
                    <div className={`flex h-7 w-7 items-center justify-center rounded-lg ${corMap[pilar.cor]} text-white`}>
                      <Icon className="h-3.5 w-3.5" />
                    </div>
                    <span className="text-xs font-semibold text-slate-700">{pilar.label}</span>
                  </div>
      ...
      <Card className="w-full">
        <CardContent className="p-4">
          <p className="text-xs font-bold uppercase text-slate-500 tracking-wide mb-3">Maturidade por módulo</p>
          <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {moduleEntries.map(([name, { progress, focus }]) => (
              <div key={name} className={`rounded-xl border p-3 ${progress >= 98 ? 'border-emerald-100 bg-emerald-50/50' : progress >= 95 ? 'border-blue-100 bg-blue-50/50' : 'border-amber-100 bg-amber-50/50'}`}>
                <div className="mb-1.5 flex items-center justify-between gap-1">
                  <span className="font-semibold text-slate-900 text-sm">{name}</span>
                  <Badge className={progress >= 98 ? 'bg-emerald-100 text-emerald-700 text-xs' : progress >= 95 ? 'bg-blue-100 text-blue-700 text-xs' : 'bg-amber-100 text-amber-700 text-xs'}>
                    {progress}%
                  </Badge>
                </div>
